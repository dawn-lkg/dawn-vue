  ...
  setup(props, { emit }) {
    ...
    function handleUpload(file) {
      return new Promise((r, j) => {
        ...
        service({ url: dataContainer.uploadApi, method: "post", data: formData, header: config }) // 使用axios发送POST请求进行文件上传，并传递配置对象包含onUploadProgress处理程序。
...
<template>
  <div class="upload-img-container">
    ...
    <div class="progress">
      <div v-if="!!dataContainer.percentCompleted" :style="{
      ...
      <div v-if="dataContainer.status == 'fail'" :style="{
      width: `100%`,
    ...
    <div v-if="dataContainer.showChooseBt" @click="handleChooseFile" :class="{
      'choose-bt': true,
    ...
    <div @click="handleCancel" v-if="dataContainer.showCancelBt" class="cancel-bt">
      <SvgIcon :style="'width: 15px;min-width:15px;height: 15px;'" name="svg:times.svg"></SvgIcon>
    </div>
  </div>
</template>
