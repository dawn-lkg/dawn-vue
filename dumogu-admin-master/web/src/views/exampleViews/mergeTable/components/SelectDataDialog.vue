<template>
  <el-dialog :title="configData.title + configData.afterTitle" v-model="configData.open" width="800px"
    :close-on-click-modal="false" append-to-body destroy-on-close :align-center="true" @close="handleClose"
      ...
      <div class="table-box content-container page-content-box">
        ...
        <div class="table-container">
          <el-table v-loading="dataContainer.loading" :data="dataContainer.list" border @cell-dblclick="handleCopyVale"
            @sort-change="handleSortChange" @row-click="(row) => {
    if (!configData.useRowClick) return;
    selectItem(row);
  }" :row-class-name="tableRowClassName" height="100%">
            <el-table-column v-if="configData.isMultiple" label="多选组件" align="center" width="40" fixed="left"
    ...
    handleSelectionChange(dataContainer.list.filter(e => e.checked__));
            ...
            <el-table-column label="昵称" show-overflow-tooltip align="center" prop="nickname" min-width="150"
              sortable="custom" :sort-orders="['descending', 'ascending']" />
            <el-table-column label="数据编号" show-overflow-tooltip align="center" prop="id" min-width="150"
              sortable="custom" :sort-orders="['descending', 'ascending']" />
        ...
        <div class="pagination-container">
          <el-pagination v-show="true" :total="dataContainer.config.total" :background="true"
            ...
            layout="total, sizes, prev, pager, next, jumper" :page-sizes="[10, 20, 30, 50]" :pager-count="7"
            @size-change="size => {
    ...
    configData.open = false;
        ...
        <el-button type="primary" v-if="configData.isMultiple && dataContainer.currentRows.length > 0"
          @click="onChange">
          确定选择
        </el-button>
      </div>
...
export default defineComponent({
  ...
  setup(_, { emit }) {
    ...
    function tableRowClassName(value) {
      let row = value.row;
      let state = false;
      if (configData.setActivedRow) {  //优先使用回调
