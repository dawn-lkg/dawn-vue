          ...
          <el-row :gutter="0">
            <el-col :span="8" :xs="8">
              <el-form-item label="菜单名称" prop="title">
                <el-input v-model="dataContainer.form.title" placeholder="请输入" clearable @clear="handleQuery"
...
export default defineComponent({
  ...
  setup() {
    ...
    const getDataList = debounceFn(function() {
      ...
      menuApi.getDataList({ ...dataContainer.form }).then(res => {
        let list = deepCopyObj(res.data || []);
        /** 
         * 将树形结构展开
         * 需要换成一维数组来过滤不要展示的目录
         *  */
    ...
    function handleDelete(data) {
      confirm('确认删除该数据？', '提示').then(() => {
        dataContainer.loading = true;
        menuApi.removeData(data.id).then(() => {
          messageSuccess("删除成功");
          getDataList();
        }).catch(() => {
          return;
